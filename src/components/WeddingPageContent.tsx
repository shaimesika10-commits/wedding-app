...
export default function WeddingPageContent({
        ...
        if (translationCache[cacheKey]) {
          ...
          try {
            ...
            schedule.forEach((e, idx) => {
              ...
              if (e.description) toTranslate.push({ idx, field: 'description', text: e.description })
            })
 ...
 %}, []) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const ShareBtn = (
    <button
      onClick={handleShare}
      className="fixed top-4 left-4 z-50 flex items-center gap-2 rounded-full px-4 py-2 shadow-lg text-sm font-medium transition-all active:scale-95 hover:shadow-xl"
      style={{
        background: shareCopied ? '#059669' : '#c9a84c',
        color: '#fff',
      }}
    ...
    >
      {shareCopied ? (
        ...
        <>
          ...
          <span>{locale === 'he' ? 'שיתוף' : locale === 'fr' ? 'Partager' : 'Share'}</span>
        </>
      )}
    </button>
  )
